...
import BusinessLogic.BiletManagerFactory;
import BusinessLogic.Exporter;
import Models.Bilet;
...
public class UserLogin {
	...
	private BiletManagerFactory bMan;
	...
	private void initialize() {
		...
		JButton btnExportaCsv = new JButton("Exporta CSV");
		btnExportaCsv.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				
				Exporter ex= bMan.getExporter("CSV");
			
				ex.export(bMan.bileteSpectacol((String)comboBox.getSelectedItem()));
				JOptionPane.showMessageDialog(null, "Reusit");
				
			}
		});
		btnExportaCsv.setBounds(490, 111, 115, 28);
		frame.getContentPane().add(btnExportaCsv);
		
		JButton btnExportaJson = new JButton("Exporta JSON");
		btnExportaJson.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				Exporter ex= bMan.getExporter("JSON");
				
				ex.export(bMan.bileteSpectacol((String)comboBox.getSelectedItem()));
				JOptionPane.showMessageDialog(null, "Reusit");
			}
		});
		btnExportaJson.setBounds(490, 150, 115, 28);
		frame.getContentPane().add(btnExportaJson);
